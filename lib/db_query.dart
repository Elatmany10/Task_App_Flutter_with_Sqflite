...
// Query to get All tasks in the Database
  Future<List<Task>> getAllTask() async {
    ...
    final List<Map<String, dynamic>> myList =
        await dbClient.query(DBHelper.taskTable);
    return List.generate(myList.length, (index) {
      ...
      return Task(
          t_id: myList[index]['t_id'],
          t_name: myList[index]['t_name'],
          time: time);
    });
  }
...
//Delete a Raw in DB
  Future<int> deleteTask(int id) async {
    var dbClient = await dbHelper.datebase;
    return await dbClient.delete(DBHelper.taskTable,
        where: '${DBHelper.t_id} = ?', whereArgs: [id]);
  }
